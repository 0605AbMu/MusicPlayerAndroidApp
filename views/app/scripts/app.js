      ...
      data: function () {
        return {
          ...
          audio1: {
            ...
            event: null,
            play: function () {
              window.document.getElementById(this.name).play();
              this.paused = false;
            },
            pause: function () {
              window.document.getElementById(this.name).pause();
              this.paused = true;
            },
            stop: function () {
              window.document.getElementById(this.name).pause();
              window.document.getElementById(this.name).currentTime = 0;
              this.paused = true;
              this.currentTime = 0;
            },
            seek: function (seconds) {
              window.document.getElementById(this.name).currentTime = seconds;
            },
            setVolume: function (volume) {
              window.document.getElementById(this.name).volume = volume;
            },
            setSource: function (mediaUrl, mediaType) {
              this.source = {"url": mediaUrl, "type": mediaType};
            },
            appendSource: function (mediaUrl, mediaType) {
              this.sources.push({"url": mediaUrl, "type": mediaType});
            },
            ...
            durationchangeHandler: function (event) {
              var
                view = app._getCurrentView(),
                views = app._getLoadedViews(),
                frames = app._getLoadedFrames(),
            ...
            errorHandler: function (event) {
              var
                ...
                self = view.audio1;
              self.event = event;
              self.error = window.document.getElementById(this.name) ? window.document.getElementById(this.name).error : null;
            ...
            volumechangeHandler: function (event) {
              var
                ...
                self = view.audio1;
              self.event = event;
              self.muted = event.target.muted;
              self.volume = event.target.volume;
...
views.view1.textarea1.value = JSON.stringify(window.android);
